...
def getStockNames(input_str):
	...
	('GET','http://dev.markitondemand.com/Api/v2/Lookup/json?input='
		+ search_str)
    if (response.status == 400):
        ...
        #filter the list based on the exchanges, and sort the output using the exchange as key
        fltrlistResponse = sorted([elem for elem in listResponse 
            if elem['Exchange'] in ['NYSE', 'NASDAQ']],key = \
			lambda f : f['Exchange'])
